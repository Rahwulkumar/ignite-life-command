...
import { format, startOfWeek, endOfWeek, eachDayOfInterval, getDay, isToday as isTodayFn } from "date-fns";
...
export function WeeklyActivityChart() {
    ...
    const weekData = useMemo(() => {
        ...
        return daysOfWeek.map(date => {
            ...
            const dayTasks = analyticsData.filter(entry => entry.entry_date === dateKey);
            ...
            const completionRate = expectedTasks > 0 ? Math.round((completedCount / expectedTasks) * 100) : 0;
                ...
                <div className="space-y-1 text-xs">
                    ...
                    {Object.keys(data.domainBreakdown).length > 0 && (
                        <>
                            ...
                            <p className="text-muted-foreground text-[10px] mb-1">By Domain:</p>
                                    ...
                                    <div key={domain} className="flex items-center gap-2">
                                        <span
                                            className={cn(
                                                ...
                                                DOMAIN_COLORS[domain as DomainId]?.text || "text-muted-foreground"
                                            )}
                                        >
                                            {domain}:
                                        </span>
        ...
        <div className="p-3 sm:p-4 h-full flex flex-col">
            ...
            <div className="flex-1 min-h-[140px]">
                <ResponsiveContainer width="100%" height={140}>
                    <BarChart data={weekData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                        <XAxis
                            dataKey="day"
                            axisLine={false}
                            tickLine={false}
