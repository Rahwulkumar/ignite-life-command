...
interface Lesson {
  title: string;
  description: string;
  domain: "tech" | "finance" | "trading" | "spiritual" | "music";
  duration: string;
  completed: boolean;
}
...
const domainStyles = {
  tech: { bg: "bg-tech/10", text: "text-tech", border: "border-tech/20" },
  finance: { bg: "bg-finance/10", text: "text-finance", border: "border-finance/20" },
  trading: { bg: "bg-trading/10", text: "text-trading", border: "border-trading/20" },
  spiritual: { bg: "bg-spiritual/10", text: "text-spiritual", border: "border-spiritual/20" },
  music: { bg: "bg-music/10", text: "text-music", border: "border-music/20" },
};
    ...
    <div className="glass rounded-xl p-5 opacity-0 animate-fade-in" style={{ animationDelay: "500ms" }}>
      ...
      <div className="space-y-3">
        {lessons.map((lesson, index) => {
          const styles = domainStyles[lesson.domain];
            ...
            >
              ...
              >
                {lesson.completed ? (
                  ...
                  <Play className={cn("w-4 h-4", styles.text)} />
                )}
              </div>
