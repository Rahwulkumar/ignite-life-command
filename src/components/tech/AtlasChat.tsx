...
export function AtlasChat() {
  const [messages, setMessages] = useState<Message[]>([
    { id: "intro", role: "assistant", content: ATLAS_INTRO }
  ]);
  ...
  return (
    <div className="flex flex-col h-[500px] border border-border/50 rounded-lg overflow-hidden bg-background">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 border-b border-border/50 bg-tech/5">
        <div className="w-10 h-10 rounded-full bg-tech/20 flex items-center justify-center">
          <Cpu className="w-5 h-5 text-tech" />
        </div>
        <div>
          <h3 className="font-medium flex items-center gap-2">
            Atlas
            <Sparkles className="w-3 h-3 text-tech" />
          </h3>
          <p className="text-xs text-muted-foreground">Tech Coach · Rigorous & Demanding</p>
        </div>
      </div>
