...
export const DailyFocusCard = () => {
  ...
  const handleLockFocus = () => {
    if (!reference || !content) {
      toast({
        ...
        description: "Please enter both reference and scripture text.",
        variant: "destructive",
      });
      return;
    }
    setFocus.mutate(
      { reference, content },
      {
        onSuccess: () => {
          toast({
            title: "Focus Locked 🔒",
            description: "This scripture is your meditation for the day.",
          });
          setIsEditing(false);
        },
      },
  ...
  const handleComplete = (completed: boolean) => {
    ...
    completeFocus.mutate(
      { id: focus.id, completed },
      {
        onSuccess: () => {
          if (completed) {
            toast({
              title: "Mastered! 🏆",
              description: "Great job hiding this word in your heart.",
            });
          } else {
            toast({
              title: "Keep going",
              description: "Review it one more time before bed!",
            });
          }
        },
      },
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Choose one scripture to master today.
            </p>
          </div>
        </div>
